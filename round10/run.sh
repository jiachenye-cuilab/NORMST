...
ROUND_ID="pro-v2-round-010"
ROUND_REASON="gene-wise Pearson alignment should improve spatial direction before variance amplitude"
...
COMMAND=(
  conda run --no-capture-output -n NORMST python -u -m round10.train
...
printf 'CUDA_VISIBLE_DEVICES=%q conda run --no-capture-output -n NORMST python -m unittest discover -s round10/tests -v\n' "${PHYSICAL_GPU}" >> "${PREFLIGHT_COMMAND_PATH}"
...
jq -n \
  ...
  --argjson smoke "${SMOKE}" \
  '{schema:"pro-normst-round10-launch-v1",status:"preflight",round_identity:$round_identity,round_reason:$round_reason,manifest:$manifest,output_dir:$output_dir,physical_gpu:$physical_gpu,gpu_state:$gpu_state,smoke:($smoke==1),log:$log,contract_test_log:$contract_test_log}' \
  > "${METADATA_PATH}"
...
BASE_TEST_STATUS=$?
if (( BASE_TEST_STATUS == 0 )); then
  CUDA_VISIBLE_DEVICES="${PHYSICAL_GPU}" conda run --no-capture-output -n NORMST \
    python -m unittest discover -s round10/tests -v >> "${PREFLIGHT_LOG}" 2>&1
  ROUND10_TEST_STATUS=$?
else
  ROUND10_TEST_STATUS=99
fi
set -e
if (( BASE_TEST_STATUS != 0 || ROUND10_TEST_STATUS != 0 )); then
  jq --argjson base_exit "${BASE_TEST_STATUS}" --argjson round10_exit "${ROUND10_TEST_STATUS}" \
    '.status="preflight-failed" | .base_test_exit_code=$base_exit | .round10_test_exit_code=$round10_exit' \
...
jq --arg sha256 "${PREFLIGHT_SHA256}" \
  '.status="running" | .base_test_exit_code=0 | .round10_test_exit_code=0 | .contract_test_log_sha256=$sha256' \
  "${METADATA_PATH}" > "${METADATA_PATH}.tmp"
mv "${METADATA_PATH}.tmp" "${METADATA_PATH}"
