...
usage() {
  echo "Usage: $0 --manifest PATH --output-dir PATH --idw-cache-dir PATH --physical-gpu INDEX [--smoke]"
}
...
for value in RUN_MANIFEST OUTPUT_DIR IDW_CACHE_DIR PHYSICAL_GPU; do
  if [[ -z "${!value}" ]]; then
...
if (( GPU_FREE < 60000 || GPU_UTIL > 20 )); then
  echo "GPU ${PHYSICAL_GPU} lacks safety margin: ${GPU_STATE}"
  exit 1
fi

ROUND_ID="pro-v2-round-012"
ROUND_REASON="dedicated local gene residual should bypass shared decoder underuse"
...
COMMAND=(
  conda run --no-capture-output -n NORMST python -u -m round12.train
  --manifest "${RUN_MANIFEST}"
  --output-dir "${OUTPUT_DIR}"
  --idw-cache-dir "${IDW_CACHE_DIR}"
  --variant full
  --seed 2027
  --round-id "${ROUND_ID}"
...
jq -n \
  ...
  --arg idw_cache_dir "$(realpath -m "${IDW_CACHE_DIR}")" \
  --argjson physical_gpu "${PHYSICAL_GPU}" \
  --arg gpu_state "${GPU_STATE}" \
  --arg log "${LOG_PATH}" \
  --arg contract_test_log "${PREFLIGHT_LOG}" \
  --argjson smoke "${SMOKE}" \
  '{schema:"pro-normst-round12-launch-v1",status:"preflight",round_identity:$round_identity,round_reason:$round_reason,manifest:$manifest,output_dir:$output_dir,idw_cache_dir:$idw_cache_dir,physical_gpu:$physical_gpu,gpu_state:$gpu_state,smoke:($smoke==1),log:$log,contract_test_log:$contract_test_log}' \
  > "${METADATA_PATH}"
...
if (( BASE_STATUS == 0 )); then
  CUDA_VISIBLE_DEVICES="${PHYSICAL_GPU}" conda run --no-capture-output -n NORMST \
    python -m unittest discover -s round10/tests -v >> "${PREFLIGHT_LOG}" 2>&1
  R10_STATUS=$?
else
  R10_STATUS=99
fi
if (( BASE_STATUS == 0 && R10_STATUS == 0 )); then
  CUDA_VISIBLE_DEVICES="${PHYSICAL_GPU}" conda run --no-capture-output -n NORMST \
    python -m unittest discover -s round11/tests -v >> "${PREFLIGHT_LOG}" 2>&1
  R11_STATUS=$?
else
  R11_STATUS=99
fi
if (( BASE_STATUS == 0 && R10_STATUS == 0 && R11_STATUS == 0 )); then
  CUDA_VISIBLE_DEVICES="${PHYSICAL_GPU}" conda run --no-capture-output -n NORMST \
    python -m unittest discover -s round12/tests -v >> "${PREFLIGHT_LOG}" 2>&1
  R12_STATUS=$?
else
  R12_STATUS=99
fi
set -e
if (( BASE_STATUS != 0 || R10_STATUS != 0 || R11_STATUS != 0 || R12_STATUS != 0 )); then
  jq \
    --argjson base_exit "${BASE_STATUS}" \
    --argjson r10_exit "${R10_STATUS}" \
    --argjson r11_exit "${R11_STATUS}" \
    --argjson r12_exit "${R12_STATUS}" \
    '.status="preflight-failed" | .base_test_exit_code=$base_exit | .round10_test_exit_code=$r10_exit | .round11_test_exit_code=$r11_exit | .round12_test_exit_code=$r12_exit' \
...
jq --arg sha256 "${PREFLIGHT_SHA256}" \
  '.status="running" | .base_test_exit_code=0 | .round10_test_exit_code=0 | .round11_test_exit_code=0 | .round12_test_exit_code=0 | .contract_test_log_sha256=$sha256' \
  "${METADATA_PATH}" > "${METADATA_PATH}.tmp"
mv "${METADATA_PATH}.tmp" "${METADATA_PATH}"
...
RUN_STATUS=$?
set -e
jq --argjson exit_code "${RUN_STATUS}" \
  '.status=(if $exit_code == 0 then "complete" else "failed" end) | .exit_code=$exit_code' \
  "${METADATA_PATH}" > "${METADATA_PATH}.tmp"
mv "${METADATA_PATH}.tmp" "${METADATA_PATH}"
exit "${RUN_STATUS}"
